...
public class AddSusSandItemModifier extends LootModifier {
    ...
    public static final Supplier<Codec<AddSusSandItemModifier>> CODEC = Suppliers.memoize(() -> RecordCodecBuilder
    ...
    public AddSusSandItemModifier(LootItemCondition[] conditionsIn, @NotNull Item item) {
        super(conditionsIn);
        this.item = item;
    }
    ...
    protected @NotNull ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext lootContext) {
        ...
        if(lootContext.getRandom().nextFloat() < 0.1f){
            generatedLoot.clear();
